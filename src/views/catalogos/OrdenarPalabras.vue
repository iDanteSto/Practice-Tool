<template>
  <v-container>
    <v-data-table :headers="headers" :items="ordenar" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Ordenar palabras</v-toolbar-title>
...
export default {
  data: () => ({
    ...
    dialogDelete: false,
    headers: [
      { text: "ID", align: "start", value: "idordenar" },
  ...
  created() {
    ...
    //
    this.url = this.$http.options.root + "audios/";
  },

  methods: {
    initialize() {
      this.$http
        .get("ordenar_palabras.all")
        .then((response) => {
          // al arreglo que declare previamente le asigno response.body que son todos los registros
          this.ordenar = response.body;
        })
    ...
    editItem(item) {
      this.editedIndex = this.ordenar.indexOf(item);
    ...
    deleteItemConfirm() {
      this.ordenar.splice(this.editedIndex, 1);
      var datos = {
      ...
      this.$http
        .put("ordenar_palabras.update/" + this.editedItem.idordenar, datos)
        .then((response) => {
          this.initialize();
        })
        .catch((error) => console.log(error));
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        Object.assign(this.ordenar[this.editedIndex], this.editedItem);
      } else {
        // guardamos
        this.$http
          .post("ordenar_palabras.add", datos)
          .then((response) => {
            // imagen
          ...
          .catch((error) => console.log(error));
        this.ordenar.push(this.editedItem);
      }
      this.close();
    },
