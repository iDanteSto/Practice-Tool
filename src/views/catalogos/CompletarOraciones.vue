<template>
  <v-container>
    <v-data-table :headers="headers" :items="completar" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Completar oraciones</v-toolbar-title>
                  ...
                  <v-form ref="form" lazy-validation>
                    <v-row>
                      ...
                      <v-col cols="12" sm="12" md="6">
                        <v-text-field
                          v-model="editedItem.frase1"
                          label="Primera parte de la oración"
                          outlined
                          dense
                        ></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="12" md="6">
                        <v-text-field
                          v-model="editedItem.frase2"
                          label="Segunda parte de la oración"
                          outlined
                          dense
                        ></v-text-field>
                      </v-col>
...
export default {
  data: () => ({
    ...
    dialogDelete: false,
    headers: [
      { text: "ID", align: "start", value: "idcompletar" },
  ...
  methods: {
    ...
    editItem(item) {
      this.editedIndex = this.completar.indexOf(item);
    ...
    deleteItemConfirm() {
      this.completar.splice(this.editedIndex, 1);
      var datos = {
        frase: this.editedItem.frase,
        frase1: this.editedItem.frase1,
        frase2: this.editedItem.frase2,
      ...
      this.$http
        .put("completar_oraciones.update/" + this.editedItem.idcompletar, datos)
        .then((response) => {
          this.initialize();
        })
        .catch((error) => console.log(error));
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        this.$http
          .put(
            "completar_oraciones.update/" + this.editedItem.idcompletar,
            datos
          )
          .then((response) => {
            this.initialize();
        ...
        Object.assign(this.completar[this.editedIndex], this.editedItem);
      } else {
        // guardamos
        this.$http
          .post("completar_oraciones.add", datos)
          .then((response) => {
            this.initialize();
          })
          .catch((error) => console.log(error));
        this.completar.push(this.editedItem);
      }
      this.close();
    },
